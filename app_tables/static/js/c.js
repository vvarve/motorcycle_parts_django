...
if (document.getElementById("form_product")) {
    ...
    form_sended.addEventListener( "submit", (e) => {
        const selects = form_sended.getElementsByTagName("select")
        if (selects[0].value.startsWith(" ") || selects[1].value.startsWith(" ") || selects[0].value.startsWith("") || selects[1].value.startsWith("")){
            e.preventDefault()
        }
    }) 
}
